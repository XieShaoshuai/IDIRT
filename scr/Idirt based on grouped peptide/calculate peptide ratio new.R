...
pep_ratio <- function(idirt,pep){
  ...
  for(acc in protein$Accession){
    ...
    if(nrow(df_temp)>1){  #check if protein is found at least 2 peptides in this buffer
      ...
      successes <- round(sum(a)+sum(b)+sum(c)+sum(d),2)
      
      post_ratio <- data.frame(pi = rbeta(2000, successes, n_trials-successes))
        #add ratio to protein----
      
      protein[protein$Accession==acc,"ratio"] <- mean(post_ratio$pi)
    }
  }
...
saveRDS(IDIRT_data,file="./data/IDIRT data/IDIRT_peptide_ratio.rsd")
