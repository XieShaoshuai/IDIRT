...
stochiometry <- function(data,k){
  ...
  p<- ggplot(data,aes(median_ratio_1,median_ratio_2,color=group,label=label))+
    ...
    theme_bw()
  ggsave(p, file=paste0("./Output/IDIRT/stochiomerty_","_Buffer",k,".pdf"),
         width = 6,
         height = 4,
         units = "in",
         dpi = 600)
  ...
  p1<- ggplot(data,aes(median_ratio_1,median_ratio_2,color=group,label=Gene))+
    ...
    ylab("I-DIRT swap:L/(H+L)")+
    theme_bw()
  ggsave(p1, file=paste0("./Output/IDIRT/stochiomerty_all_","_Buffer",k,".pdf"),
         width = 7,
         height = 5,
         units = "in",
         dpi = 600)
