...
{ 
  ...
  for (i in 1:ngene) 
  {
    ux[i] = mean(x[i,])
    uy[i] = mean(y[i,])
    var[i] = (var(as.numeric(x[i,]))*(nx-1)+var(as.numeric(y[i,]))*(ny-1))/(nxy-2)
  ...
  {print(j)
    ...
    for (i in 1:ngene)
    {
      temp0 = temp0 + (1-rd[i])*prod(dnorm(as.numeric(y[j,]),as.numeric(ux[i]),sqrt(as.numeric(var[i]))))
      temp1 = temp1 + prod(dnorm(as.numeric(y[j,]),(as.numeric(ux[i]+d[i])),sqrt(as.numeric(var[i]))))
    }
    BODP[j] = temp1/temp0
  }
